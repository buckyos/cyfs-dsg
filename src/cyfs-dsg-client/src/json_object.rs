...
pub type DsgJSONObjectType = NamedObjType<JSONDescContent, JSONBodyContent>;
pub type DsgJSONObjectBuilder = NamedObjectBuilder<JSONDescContent, JSONBodyContent>;
pub type DsgJSONObject = NamedObjectBase<DsgJSONObjectType>;

pub trait DSGJSON<T: Serialize + for<'a> Deserialize<'a>> {
    fn new(dec_id: ObjectId, owner_id: ObjectId, obj_type: u16, obj: &T) -> BuckyResult<DsgJSONObject>;
...
impl DSGJSONType for NamedObjectBase<DsgJSONObjectType> {
    fn get_json_obj_type(&self) -> u16 {
...
impl <T: Serialize + for<'a> Deserialize<'a>> DSGJSON<T> for NamedObjectBase<DsgJSONObjectType> {
    fn new(dec_id: ObjectId, owner_id: ObjectId, obj_type: u16, obj: &T) -> BuckyResult<DsgJSONObject> {
        ...
        Ok(DsgJSONObjectBuilder::new(desc, body).owner(owner_id).dec_id(dec_id).build())
    }
...
impl Verifier for NamedObjectBase<DsgJSONObjectType> {
    fn verify_body(&self) -> bool {
...
pub struct RecoveryReq {
    ...
    pub latest_state_id: String,
    pub target_id: String,
    pub target_dec_id: String,
}
